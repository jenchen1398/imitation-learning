...
class VisualModel(nn.Module):
    def __init__(self):
        ...
        fc_inshape = width*height*h + bias_len
        #breakpoint()
    ...
    def forward(self, img, state_vec, weights=None):
        ...
        if weights is None:
            ...
            bias = self.bias.unsqueeze(0).expand(img.shape[0],self.bias.shape[0]) # expand to (B,10)
            img_and_state = torch.cat([flat_img, bias],dim=1)
            ...
            bias = weights['bias']
            bias = bias.unsqueeze(0).expand(img.shape[0],bias.shape[0])
            img_and_state = torch.cat([flat_img, bias],dim=1)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        if epoch % interval == 0:
            torch.save(model, "models/fuck-model-{}.pth".format(epoch))
